...
#' @description An S4 method to detect the change-points in a high-dimensional GARCH process using the DCBS methodology described in Cho and Korkas (2018).
...
setGeneric(name="garch.seg",
           def=function(x, p=1, q=1, f=8, sig.level=0.05, 
                        Bsim=200, off.diag=TRUE, dw=NULL, do.pp=TRUE,do.parallel=TRUE)
           {
             standardGeneric("garch.seg")
           }
)
#' @rdname garch.seg-class
setMethod(f="garch.seg", definition = function(x, p=1, q=1, f=8, sig.level=0.05, 
            Bsim=200, off.diag=TRUE, dw=NULL, do.pp=TRUE,do.parallel=TRUE) {
    ...
    if(do.pp && length(est.cps)>1){
      ...
      pp.ns <- func.null.stat(pp.mat, op=2, res, T, c.mat, prob=NULL, burnin, p, q, Bsim, boot.op=3, f, eps, off.diag, sgn, mby, tby, gam)
      pp.pos.seq <- c()
